"""ngstream API. Defines the data types and provides base implementations.
"""
from abc import ABCMeta, abstractmethod
from pkg_resources import iter_entry_points, load_entry_point
from typing import (
    Tuple, Sequence, Iterator, Callable, Union, TypeVar, Generic, GenericMeta, Optional)
...
class Record(metaclass=ABCMeta):
    ...
    def as_fastq(self) -> str:
        """Convert this Record to a FASTQ record string.
        """
        return f"@{self.name}\n{self.sequence}\n+\n{self.qualities}\n"
...
class Fragment(Generic[RecordType]):
    ...
    def as_fastq(self) -> str:
        """Convert this Fragment to a FASTQ record string.
        """
...
class Protocol(Generic[RecordType], metaclass=GenericMeta):
    ...
    def __iter__(self) -> Iterator[Union[RecordType, Fragment[RecordType]]]:
        """Iterate over records/pairs. If the protocol has fragment-level read pairing
...
def dump_fastq(
    ...
    if protocol.paired and not interleaved:
        files = [
            f'{prefix or protocol.accession}.{read_idx}.fq'
            for read_idx in (1, 2)
        ]
        file_handles = [
            xopen(f, output_mode, compression=compression)
            for f in files
        ]
...
def list_protocols() -> Tuple[str, ...]:
    return tuple(set(
        entry_point.name
        for entry_point in iter_entry_points('ngstream.protocol')))
